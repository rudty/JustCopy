...
{
    private readonly MpscAsyncLinkedQueue<T> taskQueue = new();
    private readonly RecycleTask task = new();
    ...
    {
        Debug.WriteLine($"Looper OnException Item:{item} Exception:{exception}");
    }
...
internal class ThreadPoolMessageExecutor : MessageExecutorBase
{
    protected override void RunInternal(Action<object?> action, object? state)
    {
        ThreadPool.UnsafeQueueUserWorkItem(action, state, false);
    }
}

internal class NotImplementsMessageExecutor : MessageExecutorBase
{
    public static readonly NotImplementsMessageExecutor Instance = new();
    protected override void RunInternal(Action<object?> action, object? state)
    {
...
internal sealed class RecycleTask : ICriticalNotifyCompletion
{
    ...
    public void GetResult() { } // 결과가 없는 Task이므로 비워둡니다.
    ...
    {
        Debug.Assert(c is not null, "ExecuteContinuationWithContext action is not null");
