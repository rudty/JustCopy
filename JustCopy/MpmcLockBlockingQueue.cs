...
namespace JustCopy
{
    ...
    public sealed class MpmcLockBlockingQueue<T>
    {
        ...
        public MpmcLockBlockingQueue(int initializeSegmentSize = 4096)
        {
            ...
            if (initializeSegmentSize < 32)
            {
                initializeSegmentSize = 32; // 최소 크기 보장
            }
            else if (initializeSegmentSize > maxCapacity)
            {
                initializeSegmentSize = maxCapacity;
            }
            else
            {
#if NETCOREAPP3_0_OR_GREATER
                initializeSegmentSize = (int)System.Numerics.BitOperations.RoundUpToPowerOf2((uint)initializeSegmentSize);
#else
                // 2의 제곱수로 올림 (capacity가 이미 2의 제곱수라면 그대로 유지)
                initializeSegmentSize -= 1;
                initializeSegmentSize |= initializeSegmentSize >> 1;
                initializeSegmentSize |= initializeSegmentSize >> 2;
                initializeSegmentSize |= initializeSegmentSize >> 4;
                initializeSegmentSize |= initializeSegmentSize >> 8;
                initializeSegmentSize |= initializeSegmentSize >> 16;
                initializeSegmentSize += 1;
#endif
            }
